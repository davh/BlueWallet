 ...
 */
export class TestnetWallet extends LegacyWallet {
  static type = 'legacy-test';
  static typeReadable = 'Legacy (Testnet)';
  static network = 'testnet3';
    ...
    return new Promise(function(resolve) {
      if (typeof RNRandomBytes === 'undefined') {
        ...
        return crypto.randomBytes(32, (err, buf) => { // eslint-disable-line
          ...
          }, bitcoin.networks.testnet).toWIF();
          resolve();
        });
      }
  ...
  getAddress() {
    ...
    let address;
    try {
      let keyPair = bitcoin.ECPair.fromWIF(this.secret, bitcoin.networks.testnet);
  ...
  async fetchBalance() {
    try {
      const api = new Frisbee({
        baseURI: 'https://api.blockcypher.com/v1/btc/test3/addrs/',
      });
  ...
  async broadcastTx(txhex) {
    ...
    let res = await api.post('/v1/btc/test3/txs/push', { body: { tx: txhex } });
  ...
  createTx(utxos, amount, fee, toAddress, memo) {
    ...
    return signer.createTransaction(utxos, toAddress, amountPlusFee, fee, this.getSecret(), this.getAddress(), bitcoin.networks.testnet);
  }
